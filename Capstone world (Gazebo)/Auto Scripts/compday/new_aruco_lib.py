        ...
        while not self._kill:
            ...
            if ids is not None:
                ...
                if self.id_to_find in detected_ids:
                    ...
                    # Use the marker's rvec and tvec corresponding to id_to_find
                    rvec = rvecs[index][0]  # shape: (3,)
                    tvec = tvecs[index][0]  # shape: (3,)

                    # Compute rotation matrix from rvec
                    R_cam, _ = cv2.Rodrigues(rvec)  # R_cam is 3x3

                    # Convert marker translation to world coordinates (if desired)
                    t_world = -R_cam.T @ tvec   # t_world is (3,)
                    ...
                    with self._lock:
                        ...
                        self.latest['t_world'] = tuple(t_world)
                        self.latest['rvec']  = tuple(rvec)
                        self.latest['R_cam'] = R_cam

                    if verbose:
                        print(f"Detected CORRECT ID {self.id_to_find}: x={x:.1f} y={y:.1f} z={z:.1f}  fps={self.fps_detect:.0f}")
                else:
                    # Desired marker not found, but other markers were detected.
                    if verbose:

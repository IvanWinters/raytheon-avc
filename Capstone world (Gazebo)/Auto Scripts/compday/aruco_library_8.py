    ...
    def track(self, loop=True, verbose=False, show_video=None):
        ...
        while not self._kill:
            ...
            if ids is not None:
                ...
                if self.id_to_find in detected_ids:
                    # Desired marker found.
                    marker_found = True
                    self._update_fps_detect()

                    # Pose estimation for the marker matching id_to_find.
                    index = detected_ids.index(self.id_to_find)
                    rvecs, tvecs, _ = aruco.estimatePoseSingleMarkers(
                        corners, self.marker_size,
                        self._camera_matrix,
                        self._camera_distortion
                    )
                    rvec, tvec = rvecs[index][0], tvecs[index][0]
                    x, y, z = float(tvec[0]), float(tvec[1]), float(tvec[2])

                    # Draw detected markers for debugging.
                    aruco.drawDetectedMarkers(frame, corners)
                    # Optionally draw axis: aruco.drawAxis(frame, self._camera_matrix, self._camera_distortion, rvec, tvec, 0.1)

                    # Update shared state.
                    with self._lock:
                        self.latest['found'] = True
                        self.latest['tvec']  = (x, y, z)
                        # Optionally you could compute and store t_world and R_cam here.
                    
                    if verbose:
                        print(f"Detected CORRECT ID {self.id_to_find}: x={x:.1f} y={y:.1f} z={z:.1f}  fps={self.fps_detect:.0f}")

                    # If there are other markers besides the desired one, print them.
                    other_ids = [did for did in detected_ids if did != self.id_to_find]
                    if other_ids and verbose:
                        print(f"Other detected marker IDs: {other_ids}")

                else:
                    # Desired marker not found; print all detected markers.
                    marker_found = False
                    with self._lock:
                        self.latest['found'] = False
                    if verbose:
                        print(f"Desired marker {self.id_to_find} not found; detected markers: {detected_ids}")
            ...
            # Single-shot mode: if loop is False, exit after one cycle.
            if not loop:

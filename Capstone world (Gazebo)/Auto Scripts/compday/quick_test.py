...
# converts camera/uva frame coordinates to north/east coordinates
...
def marker_to_ned(gamma, R_cam, t_world):
    # gamma: drone yaw in radians
    # r_cam: camera rotation matrix
    # t_world: world translation vector
    R_fb = np.array([
...
def main():
    ...
    while True:
        ...
        if aruco_tracker.latest.get('found', False) and not aruco_discovery_logged:
            ...
            x_world, y_world, _ = aruco_tracker.latest['t_world']
            t_world = np.array(aruco_tracker.latest['t_world'])  # expect shape (3,)
            R_cam = aruco_tracker.latest['R_cam']
            ...
            # Calculate NED offsets using your marker_to_NED function.
            # Use the UAV yaw stored in scout.attitude.yaw (in radians)
            north_offset, east_offset = marker_to_ned(scout.attitude.yaw, R_cam, t_world)
            
    
            uav_loc = scout.location.global_relative_frame
            lat, lon = get_location_meters(uav_loc, north_offset, east_offset)
            ...
            #     logging.info("Sent marker coords: %s", msg)
            #     print("Sent marker coords: %s", msg)
            # except Exception as e:
            #     logging.error("Serial write failed: %s", e)

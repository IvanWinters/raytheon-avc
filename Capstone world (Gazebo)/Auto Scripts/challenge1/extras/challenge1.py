...
#Landing parameters
deg_2_rad = pi/180.0
ANGLE_DESCEND= 20 * deg_2_rad  # acceptable angle error (in radians)
FREQ_SEND = 15              # command update frequency (Hz)
FREQ_SEND_CLOSE = 13
...
def distance_to_current_waypoint():
    ...
    if nextwaypoint == 0:
        return None
    missionitem = scout.commands[nextwaypoint - 1]
    lat = missionitem.x
    lon = missionitem.y
    alt = missionitem.z
    targetWaypointLocation = LocationGlobalRelative(lat, lon, alt)
        ...
        if marker_found:
            ...
            while marker_found:
                ...
                if time.time() >= mission_start_time + FREQ_SEND:
                    if uav_location.alt >= SCOUT_ALT/2:
                        mission_start_time = time.time() # use regular FREQ_SEND
                    elif uav_location.alt < SCOUT_ALT/2:
                        mission_start_time = time.time() - FREQ_SEND_CLOSE 
                        # use faster updates for last minute corrections
                    ...
                    target_location = LocationGlobalRelative(marker_lat, marker_lon, uav_location.alt/2)
                    
                    # Log the marker location each time it is computed
                    logging.info("Marker Location: Lat = %.7f, Lon = %.7f", marker_lat, marker_lon)
                    print("Marker Location: Lat = %.7f, Lon = %.7f", marker_lat, marker_lon)
                    
                    logging.info("SCOUT Location before GoTo: Lat = %.7f, Lon = %.7f" % (uav_location.lat, uav_location.lon))
                    print("SCOUT Location before GoTo: Lat = %.7f, Lon = %.7f" % (uav_location.lat, uav_location.lon))

                    scout.simple_goto(target_location)
                    
                    logging.info("SCOUT Location after GoTo: Lat = %.7f, Lon = %.7f" % (uav_location.lat, uav_location.lon))
                    print("SCOUT Location after GoTo: Lat = %.7f, Lon = %.7f" % (uav_location.lat, uav_location.lon))

                    time.sleep(.5)
 
                    # Check if centered over marker
                    if (abs(uav_location.lat - marker_lat) <= .000001) and (abs(abs(uav_location.lon) - abs(marker_lon)) <= .000001):
                        logging.info("MARKER Location at descent: Lat = %.7f, Lon = %.7f", marker_lat, marker_lon)
                        logging.info("SCOUT Location at descent: Lat = %.7f, Lon = %.7f" % (uav_location.lat, uav_location.lon))
                        ...
                        print("COMMS Transmitted the Marker Location: Lat = %.7f, Lon = %.7f to pd drone", target_location.lat, target_location.lon)


                        print("Centered over marker. Land.")
                        logging.info("Centered over marker. Land.")

...
def send_coords_and_wait_ack(lat, lon,
                             max_retries=30,
                             retry_delay=0.5,
                             redo_msg="redo\n"):
    coord_msg = f"{lat:.7f},{lon:.7f}".encode('utf-8')
    expected_ack = coord_msg.decode('utf-8')

    for attempt in range(1, max_retries + 1):
        sock.sendto(coord_msg, ADDR)
        print(f"[Comm] Sent coords (attempt {attempt}): {coord_msg.decode()}")

        try:
            raw, _ = sock.recvfrom(1024)
            raw = raw.decode().strip()
        except Exception as e:
            print(f"[Comm] Socket error: {e}")
            continue
        ...
        if raw == expected_ack:
            ...
            print(f"[Comm] Unexpected ack (“{raw}”), sending REDO.")
            sock.sendto(redo_msg.encode(), ADDR)
        time.sleep(retry_delay)
    return False

def send_go_command():
    go_msg = "authorized".encode()
    sock.sendto(go_msg, ADDR)
    print("[Comm] Sent authorized command")
    return True
...
#        target=aruco_tracker.track,
    ...
    while True:
        ...
        if send_coords_and_wait_ack(lat, lon):
            send_go_command()      
